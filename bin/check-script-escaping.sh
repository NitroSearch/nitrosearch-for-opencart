...
#
# WHY A GREP AND NOT A CODE REVIEW. `Storefront/Widget.php` gets this right, and
# has since the module was written — it passes JSON_HEX_TAG and says why. This
# guard exists because the OTHER way of writing it is invisible:
#
#     $json = str_replace('<', '<', $json);
#
# The needle and the replacement are the same byte. That compiles, runs, escapes
# nothing, and reads exactly like a version that works. There is nothing on the
# line for a reviewer to catch — the bug is that two strings are equal — so the
# only thing that reliably catches it is a machine comparing them.
#
